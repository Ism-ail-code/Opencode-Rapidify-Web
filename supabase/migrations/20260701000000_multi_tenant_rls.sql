...
-- ============================================================================
-- This migration enhances all existing RLS policies so that ANY member of a
-- merchant (owner, admin, member) can access that merchant's records, not
-- just the owner. It also creates a helper function to centralise the
-- tenant-check logic and reduce policy duplication.
-- ============================================================================

-- -------------------------------------------------------------------------
-- 1.  Unified tenant-check helper
--     Returns TRUE when the authenticated user is either the merchant owner
--     OR a recorded member (any role) of that merchant.
-- -------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.is_merchant_member IS
  'Returns TRUE if _user_id owns or is a member of _merchant_id. Used in RLS policies.';

-- -------------------------------------------------------------------------
-- 2.  Drop and recreate RLS on products to use merchant_members
-- -------------------------------------------------------------------------
...
-- -------------------------------------------------------------------------
-- 3.  Drop and recreate RLS on product_variants to use merchant_members
-- -------------------------------------------------------------------------
...
CREATE POLICY "variants public read"
  ...
  USING (EXISTS (
    ...
    WHERE p.id = product_id AND (p.status = 'active' OR is_merchant_member(p.merchant_id, auth.uid()))
  ));
...
-- -------------------------------------------------------------------------
-- 4.  Drop and recreate RLS on analytics_events to use merchant_members
-- -------------------------------------------------------------------------
...
-- -------------------------------------------------------------------------
-- 5.  Drop and recreate RLS on processing_jobs to use merchant_members
-- -------------------------------------------------------------------------
...
-- -------------------------------------------------------------------------
-- 6.  Drop and recreate RLS on marketplace_connections to use members
-- -------------------------------------------------------------------------
...
-- -------------------------------------------------------------------------
-- 7.  Drop and recreate RLS on external_catalog_items to use members
-- -------------------------------------------------------------------------
...
-- -------------------------------------------------------------------------
-- 8.  Drop and recreate RLS on credit tables to use members
-- -------------------------------------------------------------------------
...
-- -------------------------------------------------------------------------
-- 9.  Add merchant_id to used_nonces and audit_logs for optional tenant
--     scoping, and update their RLS policies.
-- -------------------------------------------------------------------------
...
-- Only service_role can manage security tables (unchanged), but admins
-- who are also merchant members can read audit logs for their tenant.
...
-- -------------------------------------------------------------------------
-- 10. Add merchant_id to asset_cache for tenant-relationship tracking
-- -------------------------------------------------------------------------
ALTER TABLE asset_cache
  ADD COLUMN IF NOT EXISTS merchant_id uuid REFERENCES merchants(id) ON DELETE CASCADE;

-- -------------------------------------------------------------------------
-- 11. Ensure analytics_events always has a merchant_id (no orphan rows)
-- -------------------------------------------------------------------------
ALTER TABLE analytics_events
  ALTER COLUMN merchant_id SET NOT NULL;

-- -------------------------------------------------------------------------
-- 12. Index on the new columns
-- -------------------------------------------------------------------------

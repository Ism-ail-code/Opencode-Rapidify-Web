...
-- ---------------------------------------------------------------------------
-- 1.  Indexes on analytics_events for attribution queries
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2.  Attribution sessions table
--      Pre-computed per-session funnel: did they view, launch AR, add to
--      cart, purchase? Stores revenue and AR session duration for fast
...
CREATE TABLE public.attribution_sessions (
  ...
  -- Revenue in cents (from purchase_completed events metadata)
  revenue_cents   integer NOT NULL DEFAULT 0,

  -- AR session duration in seconds
...
COMMENT ON TABLE public.attribution_sessions IS
  'Pre-computed per-session funnel, updated by trigger on analytics_events insert.';
...
-- ---------------------------------------------------------------------------
-- 3.  Function: upsert attribution session
--      Merges event data into session rows. Called by the trigger below.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4.  Trigger: maintain attribution_sessions on analytics_events insert
-- ---------------------------------------------------------------------------

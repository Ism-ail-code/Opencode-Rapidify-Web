...
async function upsertProductFromWebhook(params: {
  ...
  // Upsert product — match on merchant_id + unique slug to avoid duplicates
  ...
  // Queue a processing job for 3D generation (new products only)
  if (product) {
...
// Shopify setup: Admin → Notifications → Webhook → POST to the URL above
// ---------------------------------------------------------------------------

const ShopifyWebhookSchema = z.object({
  // Shopify sends the raw JSON body — we only validate the structure
...
export const handleShopifyWebhook = createServerFn({ method: "POST" })
  .handler(async ({ request }) => {
    ...
    try {
      ...
      // products/delete — the product will be caught by the next sync,
      // but we acknowledge the webhook to keep the delivery pipeline healthy.
...
export const handleAmazonWebhook = createServerFn({ method: "POST" })
  .handler(async ({ request }) => {
    ...
    try {
      ...
      // - Listen for ANY_OFFERS_CHANGED / MFN_ORDER / PRODUCTS_ENABLED
...
// Daraz docs:    https://open.daraz.com/doc/notifications
// ---------------------------------------------------------------------------
